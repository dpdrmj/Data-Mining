...
public class AlgoPHM {
	...
	AlgoPHM()
	{
		pq = new PriorityQueue<Towrite>(new Comparator<Towrite>() {
			public int compare(Towrite l, Towrite r)
			{
				if(l.X.elements.size()<r.X.elements.size()) return 1;
				if(l.X.elements.size()>r.X.elements.size()) return -1;
				return 0;
			}
	...
	/** Number of high utility items to be mined**/
	//public int k;
	PriorityQueue<Towrite> pq;
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		while(!pq.isEmpty())
		{
			...
			if(mtemp.X.elements.size()<tempSize)
				break;
			else
				tempSize = mtemp.X.elements.size();
			//System.out.println(mtemp.X.elements.size());
			
			writeOut(mtemp.prefix, mtemp.prefixLength, mtemp.X, mtemp.averagePeriodicity);
		}
		...
		for(int i=0; i< ULs.size(); i++)
		{
			...
			if(X.sumIutils + X.sumRutils >= minUtility)
			{
				...
				if(X.sumIutils >= minUtility 
					...
					// save to file
					Towrite mtemp = new Towrite(prefix, prefixLength, X, averagePeriodicity);
					pq.add(mtemp);
