...
import fhm1_b.MemoryLogger;
import fhm1_b.Towrite;
/**
	...
	* the idea is to always reuse the same buffer to reduce memory usage. */
	...
	public AlgoFHM() 
	{
		pq = new PriorityQueue<Towrite>(new Comparator<Towrite>() {
			...
			{
				if(l.X.sumIutils<r.X.sumIutils) return 1;
				if(l.X.sumIutils>r.X.sumIutils) return -1;
				return 0;
			}
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		while(!pq.isEmpty())
		{
			...
			writeOut(mtemp.prefix, mtemp.prefixLength, mtemp.X.item, mtemp.X.sumIutils);
		}
	...
	private void fhm(int [] prefix,
		...
		for(int i=0; i< ULs.size(); i++){
			...
			if(X.sumIutils >= minUtility){
				// save to file
				Towrite temp = new Towrite(prefix, prefixLength, X);
				pq.add(temp);
